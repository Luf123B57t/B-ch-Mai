...
final_csv_path = r'/home/user01/yte_BachMai/infection_check_detailed_results.csv'
...
for batch_start in range(0, len(icu_stay), BATCH_SIZE):
    ...
    FLUSH_LIMIT = 5000
    vap_buffer, clabsi_buffer, cauti_buffer = [], [], []
    ...
    with open(tmp_vap, "w", encoding="utf-8") as f_vap, \
        ...
        for row in tqdm(
            ...
            if len(vap_buffer) >= FLUSH_LIMIT:
                f_vap.write("\n".join(vap_buffer) + "\n")
                vap_buffer.clear()
            if len(clabsi_buffer) >= FLUSH_LIMIT:
                f_clabsi.write("\n".join(clabsi_buffer) + "\n")
                clabsi_buffer.clear()
            if len(cauti_buffer) >= FLUSH_LIMIT:
                f_cauti.write("\n".join(cauti_buffer) + "\n")
                cauti_buffer.clear()
        ...
        if vap_buffer: f_vap.write("\n".join(vap_buffer) + "\n")
        if clabsi_buffer: f_clabsi.write("\n".join(clabsi_buffer) + "\n")
        if cauti_buffer: f_cauti.write("\n".join(cauti_buffer) + "\n")
...
# TỐI ƯU MERGE CSV
with open(final_csv_path, 'w', encoding='utf-8') as fout:
    for i, csv_file in enumerate(csv_files):
        with open(csv_file, 'r', encoding='utf-8') as fin:
            if i == 0:
                # File đầu tiên: Ghi toàn bộ nội dung (bao gồm header)
                fout.write(fin.read())
            else:
                # File tiếp theo: Bỏ qua dòng đầu (header)
                next(fin)
                fout.write(fin.read())
...
for kind, final_json_path in final_json_paths.items():
    ...
    # TỐI ƯU MERGE JSONL BẰNG SHUTIL (Sử dụng mode 'wb' và 'rb' để thao tác nhị phân)
    with open(final_json_path, "wb") as fout:
        for jf in json_files:
            with open(jf, "rb") as fin:
